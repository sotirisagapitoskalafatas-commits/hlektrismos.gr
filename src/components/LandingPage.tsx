...
import {
  ...
  Menu, X, CheckCircle2, Loader2, Send, Navigation,
} from 'lucide-react';
...
const SERVICES = [
  { icon: Zap, title: 'Ρεύμα', desc: 'Φθηνά Προγράμματα Ενέργειας ειδικά για σένα.', color: 'from-sky-400 to-blue-500', bg: 'bg-sky-50' },
  { icon: Flame, title: 'Αέριο', desc: 'Εξατομικευμένες λύσεις φυσικού αερίου για το σπίτι.', color: 'from-amber-400 to-orange-500', bg: 'bg-amber-50' },
  { icon: Sun, title: 'Φωτοβολταϊκά', desc: 'Καινοτομία και Βιώσιμη Ανάπτυξη, τώρα στο χώρο σου.', color: 'from-yellow-400 to-amber-500', bg: 'bg-yellow-50' },
  { icon: Car, title: 'Ηλεκτροκίνηση', desc: 'Οδηγούμε οικολογικά, κινούμαστε ηλεκτρικά.', color: 'from-emerald-400 to-teal-500', bg: 'bg-emerald-50' },
];
...
export default function LandingPage() {
  ...
  return (
    <div className="bg-gradient-to-b from-slate-50 via-white to-slate-50 text-slate-900 overflow-x-hidden">
    ...
    <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 ${
      ...
      {menuOpen && (
        <div className="lg:hidden absolute top-full inset-x-0 bg-white/95 backdrop-blur-xl border-b border-slate-200/60 py-6 px-6 flex flex-col gap-4">
          {links.map(l => (
            <a key={l.href} href={l.href} onClick={() => setMenuOpen(false)} className="text-slate-700 hover:text-sky-600 text-lg">
              {l.label}
            </a>
          ))}
          <button onClick={() => { setMenuOpen(false); onLogin(); }} className="mt-2 px-5 py-3 rounded-full bg-gradient-to-r from-sky-500 to-blue-600 text-white font-semibold">
            CRM Είσοδος
          </button>
        </div>
      )}
...
function Hero({ onLogin }: { onLogin: () => void }) {
  ...
  return (
    <section id="top" ref={heroRef} className="relative min-h-screen flex items-center justify-center overflow-hidden pt-24">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 text-center" style={{ transform: parallax(-0.1) }}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {stats.map((s, i) => (
              <div key={i} className="text-center group">
                <div className="text-4xl md:text-5xl font-bold bg-gradient-to-b from-sky-500 to-blue-600 bg-clip-text text-transparent group-hover:scale-110 transition-transform">
                  {s.value}
                </div>
...
function GreeceJourney() {
  ...
  const activeIcon = useMemo(() => L.divIcon({
    className: 'custom-marker',
    html: '<div class="marker-pulse marker-active"></div>',
    iconSize: [18, 18],
    iconAnchor: [9, 9],
  }), []);
  ...
  useEffect(() => {
    ...
    // Light, clean tile layer — CartoDB Positron
    ...
    JOURNEY_STOPS.forEach((stop) => {
      const marker = L.marker([stop.lat, stop.lng], { icon: customIcon })
        .addTo(map)
        .bindPopup(`<div style="font-weight:600;color:#0f172a;">${stop.name}</div><div style="color:#64748b;font-size:12px;">${stop.subtitle}</div>`);
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      const newActive = Math.min(JOURNEY_STOPS.length - 1, Math.floor(progress * JOURNEY_STOPS.length));
  ...
  // Update markers and pan map when active stop changes
  useEffect(() => {
    if (!mapRef.current) return;
    const map = mapRef.current;

    markersRef.current.forEach((marker, i) => {
      marker.setIcon(i === activeStop ? activeIcon : customIcon);
    });

    const stop = JOURNEY_STOPS[activeStop];
    if (stop) {
      map.flyTo([stop.lat, stop.lng], 7, { duration: 1.2, easeLinearity: 0.25 });
    }
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-12">
          ...
          <p className="text-slate-500 max-w-2xl mx-auto">
            Καθώς κυλάτε προς τα κάτω, ταξιδεύουμε σε κάθε γωνιά της Ελλάδας — από την Αθήνα στην Κρήτη,
            από την Ήπειρο στα Δωδεκάνησα. Παντού δίπλα σας.
          </p>
        </div>
        ...
        <div className="relative grid lg:grid-cols-[1fr_380px] gap-8 items-start perspective-container">
          ...
          <div className="relative rounded-3xl glass-card overflow-hidden" style={{ minHeight: '500px' }}>
            ...
            {/* 3D tilt overlay effect */}
            <div className="absolute inset-0 rounded-3xl pointer-events-none" style={{
              transform: `perspective(1000px) rotateX(${Math.min(scrollProgress * 8, 4)}deg)`,
              transformOrigin: 'center bottom',
              boxShadow: scrollProgress > 0.1 ? 'inset 0 60px 80px -40px rgba(255,255,255,0.3)' : 'none',
            }} />
            ...
            <div className="absolute bottom-4 left-4 right-4 glass-card rounded-2xl p-3 flex items-center gap-3 z-[1000]">
              ...
              <div className="flex-1 h-2 rounded-full bg-slate-200/60 overflow-hidden">
                <div className="h-full bg-gradient-to-r from-sky-500 to-cyan-400 transition-all duration-300 rounded-full" style={{ width: `${scrollProgress * 100}%` }} />
              </div>
              <span className="text-xs text-sky-600 font-mono font-bold">{Math.round(scrollProgress * 100)}%</span>
            </div>
          </div>
          ...
          <div className="lg:sticky lg:top-28 space-y-3">
            {JOURNEY_STOPS.map((stop, i) => (
              <div key={i} className={`p-4 rounded-2xl border transition-all duration-500 perspective-card ${
                i === activeStop
                  ? 'glass-card border-sky-400/50 scale-105 shadow-lg shadow-sky-500/10'
                  : i < activeStop
      ...
      <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
        ...
        <div className="relative perspective-container">
          ...
          <div className="relative glass-card rounded-3xl p-8 perspective-card" style={{ transform: 'perspective(1000px) rotateY(-5deg)' }}>
            <div className="aspect-square rounded-2xl bg-gradient-to-br from-sky-50 to-blue-50 border border-sky-100 flex items-center justify-center relative overflow-hidden">
              <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_rgba(14,165,233,0.12),transparent_60%)]" />
              <div className="relative text-center">
                <div className="w-24 h-24 mx-auto rounded-full bg-gradient-to-br from-sky-500 to-blue-600 flex items-center justify-center mb-6 shadow-2xl shadow-sky-500/30">
                  <Zap className="w-12 h-12 text-white" fill="white" />
                </div>
                <div className="text-6xl font-bold bg-gradient-to-b from-slate-900 to-slate-500 bg-clip-text text-transparent">100%</div>
                <div className="text-sky-600 text-sm mt-2 uppercase tracking-widest font-semibold">Δωρεάν Συμβουλευτική</div>
              </div>
            </div>
          </div>
        </div>
...
function Services({ services }: { services: typeof SERVICES }) {
  return (
    <section id="services" className="relative py-24 md:py-32 bg-gradient-to-b from-transparent via-sky-50/40 to-transparent">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-500 max-w-2xl mx-auto">
            Ολοκληρωμένες ενεργειακές λύσεις προσαρμοσμένες στις δικές σου ανάγκες.
          </p>
        </div>

        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((s, i) => (
            <div key={i} className="group relative p-8 rounded-2xl glass-card hover:border-sky-300/60 transition-all duration-500 hover:-translate-y-2 overflow-hidden perspective-card">
    ...
    <section id="providers" className="relative py-24 md:py-32">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-500 max-w-2xl mx-auto">
            Συνεργαζόμαστε με όλους τους μεγάλους παρόχους ενέργειας στην Ελλάδα — ρεύμα, αέριο, φωτοβολταϊκά και ηλεκτροκίνηση.
          </p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-3">
          {regions.map((r, i) => (
            <div key={i} className="px-5 py-3 rounded-full glass-card hover:border-sky-300/60 transition-all text-sm font-medium group cursor-default text-slate-700">
              <MapPin className="w-4 h-4 inline mr-2 text-sky-500/70 group-hover:text-sky-500" />
              {r}
            </div>
          ))}
        </div>
...
function Comparison() {
  return (
    <section id="compare" className="relative py-24 md:py-32 bg-gradient-to-b from-transparent via-sky-50/40 to-transparent">
      <div className="max-w-5xl mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-500 max-w-2xl mx-auto">
            Συγκρίνουμε τιμές, εκπτώσεις και ρήτρες από όλους τους παρόχους της ελληνικής αγοράς.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {[
            ...
            <div key={i} className={`relative p-8 rounded-2xl transition-all hover:-translate-y-2 perspective-card ${
              p.highlight
                ? 'glass-card border-sky-400/50 scale-105 shadow-xl shadow-sky-500/15'
                : 'glass-card'
            }`}>
              {p.highlight && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full bg-gradient-to-r from-sky-500 to-blue-600 text-xs font-semibold text-white shadow-lg">
                  Πιο δημοφιλές
                </div>
              )}
              ...
              <a href="#lead" className={`block w-full py-3 rounded-full text-center font-semibold transition-all ${
                p.highlight
                  ...
                  : 'border border-slate-200 hover:bg-sky-50 text-slate-700'
              }`}>
                Ξεκίνα Δωρεάν
              </a>
            </div>
      ...
      <div className="max-w-5xl mx-auto px-6 text-center">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {[
            ...
            <div key={i} className="group p-8 rounded-2xl glass-card hover:border-sky-300/40 transition-all perspective-card">
...
function FAQ({ faqs }: { faqs: typeof FAQS }) {
  ...
  return (
    <section id="faq" className="relative py-24 md:py-32 bg-gradient-to-b from-transparent via-sky-50/40 to-transparent">
...
function LeadForm() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!form.full_name || !form.phone) {
      setStatus('error'); setErrorMsg('Συμπλήρωσε όνομα και τηλέφωνο.');
      return;
    }
    setStatus('loading');
    if (supabase) {
      ...
      if (error) {
        setStatus('error'); setErrorMsg('Παρουσιάστηκε σφάλμα. Δοκιμάστε ξανά.');
        return;
      }
    }
    setStatus('success');
          ...
          <div className="relative">
            ...
            {status === 'success' ? (
              <div className="text-center py-12">
                ...
                <button onClick={() => setStatus('idle')} className="px-6 py-3 rounded-full border border-slate-200 hover:bg-sky-50 transition-all text-slate-700">
                  Νέα Υποβολή
                </button>
              </div>
            ) : (
              <form onSubmit={submit} className="grid sm:grid-cols-2 gap-4">
                ...
                {status === 'error' && (
                  <div className="sm:col-span-2 text-red-500 text-sm">{errorMsg}</div>
                )}
...
function FinalCTA({ onLogin }: { onLogin: () => void }) {
  return (
    <section className="relative py-24 md:py-32 bg-gradient-to-b from-transparent via-sky-50/40 to-transparent">
      <div className="max-w-4xl mx-auto px-6 text-center">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          ...
          <button onClick={onLogin} className="text-sm text-slate-400 hover:text-sky-600 transition-colors mt-2 sm:mt-0">
            Σύνδεση CRM →
          </button>
        </div>
      </div>
...
function Footer({ onLogin }: { onLogin: () => void }) {
  return (
    <footer className="relative py-16 border-t border-slate-200/60">
      <div className="max-w-7xl mx-auto px-6">
